...
first_pool = tf.nn.max_pool(first_conv_activation, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
...
second_pool = tf.nn.max_pool(second_conv_activation, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')


# Third convolution
third_filter = tf.get_variable('filter_3', [4, 4, 128, 256], initializer=tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32), dtype=tf.float32)
third_convolution = tf.nn.conv2d(input=second_pool, filter=third_filter, strides=[1, 1, 1, 1], padding='SAME')
third_conv_bias = tf.Variable(tf.constant(0.1, shape=[256]))
third_conv_activation = tf.nn.relu(third_convolution + third_conv_bias)
third_pool = tf.nn.max_pool(third_conv_activation, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')


# Fully connected first
pool_flat = tf.reshape(third_pool, [-1, 4 * 4 * 256])
full_connected_weights = tf.Variable(tf.truncated_normal([4 * 4 * 256, 4028], stddev=0.1))
...
with tf.Session() as sess:
    ...
    for i in range(160000):
        train_data = []
        train_labels = []
        train_data, train_labels = cifar10Data.train_data_batch(128)
        if (i % 100) == 0:
